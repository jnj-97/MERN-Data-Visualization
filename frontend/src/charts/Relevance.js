...
export const Relevance = () => {
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: `Relevance according to ${field}`,
      },
      zoom: {
        // Zoom options
        pan: {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let chartData = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}data/relevance`
      );
            ...
            {data && (
              <Bar
                ...
                data={{
                  ...
                  datasets: [
                    {
                      label: "Relevance", // Add label for the dataset
                      data: data[field].map((row) => row.totalRelevance),
